(function() {
    ...
    // ── State ──
    let fileItems = []; // { file, kind: 'image'|'text'|'pdf', dataURL?, textContent? }
    let pdfBlob = null;
    ...
    function loadFiles(files) {
        ...
        if (validFiles.length === 0) {
            statusText.textContent = 'Please select supported file types (images, text, PDF).';
            setTimeout(() => { statusText.textContent = 'Preparing your files…'; }, 2000);
            return;
        }
                ...
                if (item.kind === 'image') {
                    ...
                    if (text.trim().length > 0) {
                        ...
                        while (lineIndex < lines.length) {
                            ...
                            pdf.text(linesToAdd, margin, margin + lineHeight, { baseline: 'top' });
